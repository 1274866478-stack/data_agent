...
**职责**: 验证工具输出格式，检查数据完整性，自动修复损坏的 JSON 数据
**作者**: Data Agent Team
**版本**: 1.0.0
**变更记录**:
...
## [INPUT]
### DataValidator.validate_tool_output()
- **output**: str - 工具输出的字符串（通常是 JSON）
- **tool_name**: str - 工具名称

## [OUTPUT]
### 返回值
- **(is_valid, parsed_data, error_message)**: 验证结果元组
  - is_valid: 数据是否有效
...
## [LINK]
**上游依赖**:
- [json](https://docs.python.org/3/library/json.html) - JSON 解析

**下游依赖**:
- [./sql_agent.py](./sql_agent.py) - 在 build_visualization_response 中调用

## [POS]
**路径**: Agent/data_validator.py
...
class DataValidator:
    ...
    def _fix_comments(self, s: str) -> Optional[str]:
        """移除 JavaScript 风格的注释"""
        # 移除单行注释
        fixed = re.sub(r'//.*?(?=\n|$)', '', s)
        # 移除多行注释
