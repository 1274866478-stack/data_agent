...
class SeriesConfig(BaseModel):
    """单系列配置（用于多系列图表）"""
    column: str = Field(description="数据列名")
    chart_type: str = Field(default="line", description="图表类型: line, bar, scatter等")
    y_axis_index: int = Field(default=0, description="Y轴索引: 0=左Y轴, 1=右Y轴")
    unit: str = Field(default="", description="单位标签")
...
class ChartConfig(BaseModel):
    ...
    # 多系列支持
    series: List[SeriesConfig] = Field(default_factory=list, description="多系列配置列表")
    is_dual_axis: bool = Field(default=False, description="是否使用双Y轴")
    left_axis_name: str = Field(default="", description="左Y轴名称")
    right_axis_name: str = Field(default="", description="右Y轴名称")
