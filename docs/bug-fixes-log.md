...
### BUG-034: SQL查询结果先显示全0表格再显示正确数据 - Decimal类型JSON序列化失败
...
#### 问题描述
用户询问"2025年的销售趋势如何"时，AI助手返回了**两次查询结果表格**：
1. 第一次显示12个月的数据，但所有值都是0
2. 第二次显示正确的销售数据
...
月份    订单数量  总销售额  平均订单金额
2025-01   2182   9898143   4536.27
2025-02   1898   8407056   4429.43
...（正确的数据）
```

#### 根本原因

**问题**: PostgreSQL返回的数据中包含 `Decimal` 类型（用于高精度数值如金额、平均值），但Python标准 `json` 库无法序列化 `Decimal` 类型。

**错误日志**:
```
SQL查询执行成功...12 行
执行SQL查询失败: Object of type Decimal is not JSON serializable
尝试使用AI修复SQL...
AI修复SQL失败
```

**问题位置**: `backend/src/app/api/v1/endpoints/llm.py` 第1963行（修复前）

```python
# ❌ 问题代码 - Decimal类型无法被json.dumps序列化
data_json = json.dumps(data_for_analysis, ensure_ascii=False, indent=2)
```

**问题链条**:
1. SQL执行成功，返回12行数据（包含 `Decimal` 类型的金额字段）
2. 在构建二次LLM调用的分析提示时，尝试将数据序列化为JSON
3. `json.dumps()` 遇到 `Decimal` 类型抛出异常
4. 系统误以为SQL有问题，尝试用AI修复SQL（但根本不是SQL的问题）
5. AI修复失败，前端收到不完整的数据
6. 用户看到第一个全是0的表格（可能是默认值或部分数据），然后是正确的数据

#### 解决方法

**修复1: 添加 Decimal 导入**

文件: `backend/src/app/api/v1/endpoints/llm.py` 第8行
...
**修复2: 添加辅助函数处理 Decimal 类型**

文件: `backend/src/app/api/v1/endpoints/llm.py` 第42-59行（新增）
...
文件: `backend/src/app/api/v1/endpoints/llm.py` 第1985-1987行
...
月份     订单数量  总销售额  平均订单金额
2025-01    0        0         0        ← 错误的空数据
...

月份     订单数量  总销售额  平均订单金额
2025-01   2182   9898143   4536.27   ← 正确的数据
...
```
...
月份     订单数量  总销售额  平均订单金额
2025-01   2182   9898143   4536.27   ← 只显示一次正确的数据
2025-02   1898   8407056   4429.43
...
```
...
1. `backend/src/app/api/v1/endpoints/llm.py`
   ...
   - 新增 `_convert_decimal_to_float()` 辅助函数
   - 在二次LLM调用的数据序列化前调用转换函数

#### 预防措施

**PostgreSQL数据类型处理规范**:
1. ✅ **聚合函数返回值类型**: PostgreSQL的 `SUM()`, `AVG()` 等函数返回 `Decimal` 类型
2. ✅ **JSON序列化前转换**: 使用辅助函数将 `Decimal` 转换为 `float`
3. ✅ **递归处理嵌套数据**: 确保列表和字典中的 `Decimal` 也被转换

**常见需要转换的数据类型**:
```python
# PostgreSQL类型 → Python类型 → JSON兼容类型
DECIMAL/NUMERIC → Decimal → float
BIGINT → int → int (无需转换)
DATE/TIMESTAMP → datetime → str (使用 default=str)
```
...
#### 相关问题
- **BUG-031**: 文档管理页面 `stats.total_size_mb.toFixed is not a function` 错误，也是因为 `Decimal` 类型序列化问题

#### 验证
- ✅ 后端服务重启成功
- ✅ SQL查询成功执行，不再报 JSON 序列化错误
- ✅ 查询结果只显示一次正确的数据
- ✅ 二次LLM调用（数据分析和图表生成）正常工作
...
## BUG-037: Excel/DuckDB 日期 LIKE 查询返回 0 结果 + 饼图分布截断问题

**发现时间**: 2025-01-08
**影响模块**: Agent SQL 生成引擎
**严重程度**: 🔴 高 - 影响所有 Excel 数据源的日期查询和分布统计可视化

### 问题描述

**问题 1: 日期 LIKE 查询返回空结果**

用户询问 "2024年5月份的订单有哪些？" 时，AI 生成的可视化图表显示 0 值。

生成的 SQL:
```sql
SELECT order_id, customer_id, customer_name, order_date, total_amount, status_cn, ship_city
FROM 订单表
WHERE order_date LIKE '2024-05%'  -- ❌ 问题所在
ORDER BY order_date DESC;
```

**问题 2: 饼图分布不完整**

用户期望饼图显示**所有品牌**的分布，但 AI 使用 LIMIT 截断了结果，只显示前几个品牌。

### 根本原因

**问题 1 根因**:
1. Excel 文件通过 DuckDB 引擎读取时，日期列存储为**数值类型**（Excel 序列号）或 DATE/TIMESTAMP 类型
2. LIKE 运算符是**字符串匹配运算符**，无法用于数值/日期类型
3. 对于 DuckDB/Excel，必须使用数据库特定的日期函数

**问题 2 根因**:
1. AI 默认对查询添加 LIMIT 以限制结果数量
2. 分布统计（饼图）需要显示**所有类别**才能正确展示占比
3. 只有用户明确说 "前5名"、"Top 10" 时才应该使用 LIMIT

### 修复方案

#### 1. 系统提示词增强 - 特定年月查询

在 `Agent/sql_agent.py` 中添加明确的日期查询指南:

```python
### 🔴🔴🔴 特定年月查询（重要！）
当用户问"2024年5月"、"2023年12月订单"等**特定年月**查询时：

| 用户说 | 应理解为 | PostgreSQL 示例 | DuckDB/Excel 示例 |
|--------|----------|-----------------|-------------------|
| "2024年5月" | 2024年5月整月 | `TO_CHAR(date_col, 'YYYY-MM') = '2024-05'` | `strftime(date_col, '%Y-%m') = '2024-05'` |
| "2023年12月" | 2023年12月整月 | `date_col >= '2023-12-01'::date AND date_col < '2024-01-01'::date` | `strftime(date_col, '%Y-%m') = '2023-12'` |
| "2024年的订单" | 2024年全年 | `EXTRACT(YEAR FROM date_col) = 2024` | `EXTRACT(YEAR FROM date_col) = 2024` |
...
在 `Agent/sql_agent.py` 中添加完整的查询处理流程:
...
在 `Agent/prompt_generator.py` 的 `generate_sql_fix_prompt_with_db_type()` 函数中添加:
...
| 文件 | 修改位置 | 修改内容 |
|------|----------|----------|
| `Agent/sql_agent.py` | 第 ~277-293 行 | 添加"特定年月查询"指南，禁止 LIKE 查询日期 |
| `Agent/sql_agent.py` | 第 ~357-410 行 | 添加"详细查询+分布统计"流程和饼图规则 |
| `Agent/prompt_generator.py` | 第 ~385-395 行 | SQL 修复提示词添加日期 LIKE 修复指南 |
...
AI: 生成 SQL（使用 LIKE）
WHERE order_date LIKE '2024-05%'  -- ❌ 对日期类型无效
...
AI: 生成 SQL（使用 strftime）
WHERE strftime(order_date, '%Y-%m') = '2024-05'  -- ✅ 正确

结果: 返回 128 行订单，图表正常显示
饼图: 按状态/城市统计，包含所有类别
```
...
**SQL 生成规范**:
1. ✅ **日期过滤必须使用数据库特定函数** - PostgreSQL 用 TO_CHAR，DuckDB 用 strftime
2. ✅ **禁止对日期类型使用 LIKE 运算符** - 这是最常见的错误模式
3. ✅ **分布统计（饼图）不使用 LIMIT** - 除非用户明确要求 "前N名"

**数据库日期函数速查表**:

| 操作 | PostgreSQL | DuckDB/Excel | MySQL | SQLite |
|------|------------|--------------|-------|--------|
| 年月格式化 | `TO_CHAR(d, 'YYYY-MM')` | `strftime(d, '%Y-%m')` | `DATE_FORMAT(d, '%Y-%m')` | `strftime('%Y-%m', d)` |
| 年份提取 | `EXTRACT(YEAR FROM d)` | `EXTRACT(YEAR FROM d)` | `YEAR(d)` | `CAST(strftime('%Y', d) AS INT)` |
| 月份提取 | `EXTRACT(MONTH FROM d)` | `EXTRACT(MONTH FROM d)` | `MONTH(d)` | `CAST(strftime('%m', d) AS INT)` |
| 日期截断 | `DATE_TRUNC('month', d)` | `DATE_TRUNC('month', d)` | `DATE_FORMAT(d, '%Y-%m-01')` | `strftime('%Y-%m-01', d)` |

### 验证

- ✅ 后端 Docker 容器重启成功: `docker restart dataagent-backend`
- ✅ 查询 "2024年5月份的订单" 返回正确结果
- ✅ 饼图显示完整的分布数据（所有状态/城市/品牌）
- ✅ SQL 自动修复机制识别并修复 LIKE 日期查询

### 相关问题

- **BUG-036**: 初步添加了数据库感知的 SQL 修复机制
- **prompt_generator.py**: database_spec 模块提供各数据库函数映射
- **database_spec.py**: 定义了各支持的数据库类型的日期/聚合/字符串函数
