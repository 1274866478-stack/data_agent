...
# ============================================================================
# AgentV2 路径配置 - 确保 AgentV2 模块可导入
# ============================================================================
# 注意: sys 和 Path 已在文件顶部导入

# 添加项目根目录到 sys.path（用于导入 AgentV2）
# 从当前文件位置向上查找项目根目录
# 文件路径: backend/src/app/api/v2/endpoints/query_stream_v2.py
# 需要向上到: Project/insight-agent (包含 AgentV2 符号链接)
_project_root = Path(__file__).resolve().parent.parent.parent.parent.parent.parent
if _project_root.name == "backend":
    _project_root = _project_root.parent

# 确保找到的项目根目录包含 AgentV2 符号链接
if not (_project_root / "AgentV2").exists():
    # 如果没有找到，尝试向上再找一层
    _project_root = _project_root.parent
